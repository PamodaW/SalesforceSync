...
app.post('/getContacts', async (req, res) => {
    ...
    try {
        const response = await fetch( `${instanceUrl}/services/data/v54.0/query/?q=SELECT+Id,FirstName,LastName,CreatedDate+FROM+Contact+ORDER+BY+CreatedDate+DESC+LIMIT+20`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${accessToken}`,
            },
